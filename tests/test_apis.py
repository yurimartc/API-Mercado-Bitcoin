...
class TestTradesApi:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("TEST", None, datetime.datetime(2019, 1, 1), "https://www.mercadobitcoin.net/api/TEST/trades")
        ]
    )
    def test_get_endpoint(self, coin, date_from, date_to, expected):
        actual = TradesApi(coin=coin)._get_endpoint(date_from, date_to)
        assert actual == expected
...
class TestMercadoBitcoinApi:
    ...
    def test_get_data_requests_is_called(self, mock_get_endpoing, mock_requests, fixture_mercado_bitcoin_api):
    ...
    @patch("mercado_bitcoin.apis.MercadoBitcoinApi._get_endpoint", return_value="valid_endpoint")
    def test_get_date_valid_endpoint(self, mock_get_endpoing, mock_requests, fixture_mercado_bitcoin_api):
        actual = fixture_mercado_bitcoin_api.get_data()[0]
        expected = {"foo": "bar"}
        assert actual == expected
    ...
    def test_get_date_valid_endpoint(self, mock_get_endpoing, mock_requests, fixture_mercado_bitcoin_api):
        with pytest.raises(Exception):
            fixture_mercado_bitcoin_api.get_data()[0]
