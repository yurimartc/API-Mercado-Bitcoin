...
class DynamoCheckpoints:
    def __init__(self, model: Model, report_id: str, default_start_date= datetime.date) -> None:
    ...
    def create_checkpoint(self, checkpoint_date):
        checkpoint = self.model(report_id=self.report_id, checkpoint_date=f"{checkpoint_date}")
        checkpoint.save()
    ...
    def create_or_update_checkpoint(self, checkpoint_date):
        ...
        if not self.checkpoint_exist:
            self.create_checkpoint(checkpoint_date=checkpoint_date)
        else:
            self.update_checkpoint(checkpoint_date=checkpoint_date)
...
DynamoCheckpoints(model=CheckpointModel, report_id='abc', default_start_date=datetime.date(2022, 4, 27))